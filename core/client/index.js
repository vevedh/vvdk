...
export default async function initialize () {
  ...
  logger.debug('[VVDK] initializing core module')

  // Declare the module intiaization states
  Store.set('vvdk', { core: { initialized: false }, map: { initialized: false } })
  ...
  _.forEach(_.get(config, 'readers.core', []), entry => {
    logger.debug(`[VVDK] registering mime types [${entry.mimeTypes}] to reader ${entry.reader}`)
    Reader.register(entry.mimeTypes, readers[entry.reader])
  })

  // Store the intiaization state
  Store.set('vvdk.core.initialized', true)
}
